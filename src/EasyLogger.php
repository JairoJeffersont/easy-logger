...
class EasyLogger {
     ...
     * 
     * The log message includes a timestamp, a unique ID, and the specified log level (INFO, ERROR, DEBUG, etc.).
     ...
     * 
     * @return string Returns the unique ID generated for the log entry, allowing tracking of the specific log message.
     * 
     ...
     */
    public static function newLog(string $filename, string $message, string $level = 'INFO'): string {
        ...
        // Format the log message, including the timestamp, log level, and unique ID
        $formattedMessage = sprintf("[%s] [%s] [%s] %s%s", date('Y-m-d H:i:s'), strtoupper($level), $uniqueId, $message, PHP_EOL);

        // Write the formatted message to the log file, appending it without overwriting
        file_put_contents($logFile, $formattedMessage, FILE_APPEND | LOCK_EX);

        // Return the unique ID generated for tracking
        return $uniqueId;
    }
}
